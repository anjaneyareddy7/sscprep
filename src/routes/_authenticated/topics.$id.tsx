...
import { Youtube, Plus, Trash2, ExternalLink, RotateCw, Bookmark, Star, ChevronLeft, NotebookPen, Loader2 } from "lucide-react";
  ...
  const topicQ = useQuery({
    ...
    queryFn: async () => {
      const { data, error } = await supabase
        .from("topics")
        .select("*, section:sections!inner(*, subject:subjects!inner(*))")
  ...
  return (
    <>
      ...
      <div className="p-4 sm:p-6 max-w-5xl mx-auto space-y-6">
        {topic && (
          <Link to="/subjects/$slug" params={{ slug: topic.section.subject.slug }} className="text-sm text-muted-foreground hover:text-foreground inline-flex items-center gap-1">
            <ChevronLeft className="h-4 w-4" /> {topic.section.subject.name} · {topic.section.name}
          </Link>
        )}

        <section>
          <div className="flex items-center justify-between mb-3">
            <h2 className="font-display text-lg font-semibold flex items-center gap-2"><Youtube className="h-5 w-5 text-red-500" /> Video lectures</h2>
            {isAdmin && (
                ...
                <DialogContent>
                  ...
                  <div className="space-y-3">
                    <div>
                      ...
                      <p className="text-xs text-muted-foreground mt-1">Title, channel and thumbnail are fetched automatically when possible.</p>
                    </div>
                    ...
                    <div className="grid grid-cols-2 gap-2">
                      ...
                      <div><Label>Duration (optional)</Label><Input value={vidDuration} onChange={(e) => setVidDuration(e.target.value)} placeholder="e.g. 28:14" /></div>
                    </div>
                  </div>
          ...
          {(videosQ.data?.length ?? 0) === 0 ? (
            <EmptyState icon={<Youtube className="h-5 w-5" />} title="No videos yet" description={isAdmin ? "Add the first YouTube lecture for this topic." : "An admin can add lectures here."} />
          ) : (
                    ...
                    <CardContent className="p-3">
                      <div className="flex items-start justify-between gap-2">
                        <div className="min-w-0">
                          <h3 className="font-medium line-clamp-2">{v.title || "Untitled video"}</h3>
        ...
        <section className="grid md:grid-cols-2 gap-4">
          <Card>
            <CardContent className="p-4 space-y-3">
              <h3 className="font-display font-semibold flex items-center gap-2"><RotateCw className="h-4 w-4 text-primary" /> Revision</h3>
              <p className="text-xs text-muted-foreground">Log every time you revise this topic. Cycle #{(revisionsQ.data?.length ?? 0) + 1} next.</p>
          ...
          <Card>
            <CardContent className="p-4 space-y-3">
              <h3 className="font-display font-semibold flex items-center gap-2"><NotebookPen className="h-4 w-4 text-primary" /> Your remark</h3>
              <Textarea
...
function RevisionForm({ onSubmit, pending }: { onSubmit: (notes: string) => void; pending: boolean }) {
  ...
  return (
    <div className="space-y-2">
      <Textarea value={notes} onChange={(e) => setNotes(e.target.value)} placeholder="Quick notes from this revision (optional)" rows={2} />
